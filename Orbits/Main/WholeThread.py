...
class OrbitsThread:
    def __init__(self, name, kml_root, from_date, to_dates, range_locs, min_res, Widths, RES, Dome_widths, Fore_widths):
        """
        至少应该有一个国内卫星和一个国外卫星
        ...
        self.Widths = Widths
        self.RES = RES
        self.Dome_widths = Dome_widths
        self.Fore_widths = Fore_widths
    ...
    def merges(self):
        Dome_lists = self.Dome_widths.keys()
        Fore_lists = self.Fore_widths.keys()
        for end_date in self.to_dates:
            dome_file_paths = []
            fore_file_paths = []
            dome_file_names = []
