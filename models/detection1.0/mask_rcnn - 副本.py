...
def maskrcnn_resnet50_fpn(pretrained=False, progress=True,
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['maskrcnn_resnet50_fpn_coco'],
                                              progress=progress)
        #pop when first layer is not the same
        state_dict.pop('backbone.body.conv1.weight')

        model.load_state_dict(state_dict, False)
    return model

def dcn_maskrcnn_resnet50_fpn(num_classes=91, pretrained_backbone_path=None, pretrained=True, **kwargs):
    from .backbone_utils import dcn_fpn_backbone
    from mmcv.runner import load_checkpoint
    backbone = dcn_fpn_backbone()
    model = MaskRCNN(backbone, num_classes, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['maskrcnn_resnet50_fpn_coco'],
                                              progress=True)
    ...
    if pretrained_backbone_path:
        checkpoint = load_checkpoint(model, pretrained_backbone_path)
    return model