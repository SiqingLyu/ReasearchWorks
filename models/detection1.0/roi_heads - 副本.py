...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    # print('==============----------->',class_logits, class_logits.shape, labels.shape)
    ...
    # print(box_regression.shape, torch.max(labels_pos), torch.max(sampled_pos_inds_subset))

#######   labels_tmp = torch.where(labels_pos > 0, torch.ones_like(labels_pos), torch.zeros_like(labels_pos))
    box_loss = F.smooth_l1_loss(
#######        box_regression[sampled_pos_inds_subset, labels_tmp],
        regression_targets[sampled_pos_inds_subset],
        reduction="sum",
    )
    box_loss = box_loss / (labels.numel()+0.00001)
...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs):
    ...
    if mask_targets.numel() == 0:
        print('mask_targets.numel() == 0')
        return mask_logits.sum() * 0
    labels_tmp = torch.where(labels>0,torch.tensor(1),torch.tensor(0))
    # print('===========->', torch.isnan(mask_logits[torch.arange(labels.shape[0], device=labels.device), labels_tmp][-2:]).any())
    # print('===========->', torch.isnan(mask_logits[-2:]).any(),'\n', torch.isnan(labels[-2:]).any())
    # print(mask_logits[torch.arange(labels.shape[0], device=labels.device),labels].shape, mask_targets.shape)
    # print(mask_logits.shape)
    mask_loss = F.binary_cross_entropy_with_logits(
        mask_logits[torch.arange(labels.shape[0], device=labels.device), labels_tmp], mask_targets
    )
    return mask_loss
...
def _onnx_heatmaps_to_keypoints(maps, maps_i, roi_map_width, roi_map_height,
    ...
    width_correction = widths_i / (roi_map_width+0.00001)
    height_correction = heights_i / (roi_map_height+ 0.00001)
...
class RoIHeads(torch.nn.Module):
    ...
    def select_training_samples(self, proposals, targets):
        ...
        gt_boxes = [t["boxes"].to(dtype).cuda() for t in targets]
    ...
    def postprocess_detections(self, class_logits, box_regression, proposals, image_shapes):
        ...
        for i in boxes_per_image:
            if i != 0:
                tmp = False
        if tmp:
            all_boxes.append(torch.zeros(1, 4))
            all_scores.append(torch.zeros(2))
            all_labels.append(torch.zeros(2).type(torch.long))
            all_boxes = torch.tensor([item.cpu().detach().numpy() for item in all_boxes]).cuda()
            all_labels = torch.tensor([item.cpu().detach().numpy() for item in all_labels]).cuda()
...
#####        class_logits = torch.where(class_logits!=class_logits, torch.zeros_like(class_logits).cuda(), class_logits)
        ...
        if self.training:
            ...
            for i in range(num_images):
                result.append(
                    {
                        "boxes": proposals[i] if self.gt_rpn else boxes[i],
                        "labels": labels[i],
                        "scores": scores[i],
                    }
                )
        ...
        if True:
            if self.training:
                ...
                for img_id in range(num_images):
                    ...
                    nos_proposals.append(proposals[img_id][pos])
                    pos_matched_idxs.append(matched_idxs[img_id][pos]) 
            else:
                nos_proposals =  [p["boxes"] for p in result]
                pos_matched_idxs = None
            
            #nos_proposals = self.scale_proposals(nos_proposals)
            nos_features = self.nos_roi_pool(features, nos_proposals, image_shapes)
            nos_features = self.nos_head(nos_features)
            nos_predict = self.nos_predictor(nos_features, nos_proposals)
            #拆分一个batch的nos_predict到各个patch
            
            if self.training:
                gt_nos = [t["nos"][p].cuda() for t, p in zip(targets, pos_matched_idxs)]
                fname = [t["file_name"] for t in targets]
                rcnn_loss_nos = self.nos_loss(gt_nos, nos_predict, fname)
                losses['loss_nos'] = rcnn_loss_nos
            else:
        ...
        if self.has_mask():
            ...
            if self.training:
                ...
                gt_labels = [t["labels"] for t in targets]
                #TODO 这里的mask在训练时也可以假设存在，怎么做，需要debug一下
                rcnn_loss_mask = maskrcnn_loss(     # in this function gt_labels has been transformed into 0-1 fomat matrix
