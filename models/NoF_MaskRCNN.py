...
class MaskRcnn_res50(MaskRCNN):
    def __init__(self, params):
        ...
        self.save_path = params['savepath']
        ...
        if not self.gt_rpn_training:
            ...
            for inx, (img, target, predict) in enumerate(zip(images, gts_post, output)):
                ...
                if 'masks' in predict.keys():
                    ...
                    if save_opt:
                        self.save_predict(predict, target['file_name'], self.exp_folder)
    ...
    def test_step(self, batch):
        return self.validation_step(batch, True)

    def save_predict(self, predict, filename, save_path):
        mask = np.squeeze(predict['masks'].cpu().detach().numpy())
        labels = predict['nos'].cpu().detach().numpy()
        if len(labels) == 0:
            # print("no item found")
            return
        # print(mask.shape, labels)
        mask_all = np.zeros((self.img_size, self.img_size))
        # assert len(mask) == len(labels)
        for i in range(len(labels)):
            mask_save = mask[i]
            label_tmp = labels[i]
            mask_all = np.where(mask_save > self.iou_threshold, label_tmp, mask_all)
        tif.imsave(save_path + '/' + filename + '.tif', mask_all)
