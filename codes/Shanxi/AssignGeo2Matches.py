...
if __name__ == '__main__':
    ...
    #
    # # 结果存储位置
    save_dir = make_dir(r'D:\experiment\2024\Assigned_Test\IntForm')
    # main(from_dir, to_dir, save_dir)
    names = ['Changsha_0_14.tif', 'Huizhou_0_0.tif', 'Jiaxing_0_3.tif', 'Jinhua_0_6.tif', 'Lanzhou_0_0.tif',
             'Luoyang_0_2.tif', 'Nantong_0_0.tif', 'Qingdao_0_15.tif', 'Shaoxing_0_0.tif', 'Shenyang_0_13.tif',
             'Wuhu_0_0.tif', 'Wuxi_0_0.tif', 'Xuzhou_0_0.tif', 'Zhuhai_0_0.tif']
    cities = ["Changsha","Huizhou","Jiaxing","Jinhua", "Lanzhou", "Luoyang", "Nantong", "Qingdao"
              ,"Shaoxing", "Shenyang", "Wuhu", "Wuxi", "Xuzhou", "Zhuhai"]
    for ii in range(len(names)):
        path = os.path.join(r'D:\experiment\2024\Assigned_Test', cities[ii], names[ii])
        new_path = os.path.join(r'D:\experiment\2024\Assigned_Test', cities[ii], cities[ii]+'.tif')
        data = tif.imread(path)
        data = np.round(data).astype(np.uint8)
        tif.imsave(new_path, data)
        asign(from_img=new_path, to_img=path, out_path=os.path.join(save_dir, '{}.tif'.format(cities[ii])))