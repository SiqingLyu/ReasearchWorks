...
if __name__ == '__main__':
    ...
    save_path = r'G:\ProductData\CBRA\CBRA_clip_rectangle'  # 裁剪得到的影像的保存位置
    save_name = "Beijing"  # 裁剪得到的影像系列的名称, 最终结果默认存在save_name0.tif中
    # 获取目标的经纬度范围
    for info in excel_data:

        # TODO: 在实际使用的时候，通过这几行代码将excel表格里的数据读取，得到经纬度范围（左下角和右上角两点经纬度坐标）
        #  Lon_min, Lat_min, Lon_max, Lat_max = info
        Lon_min, Lat_min, Lon_max, Lat_max = 117.1, 40.5, 117.5, 40.9  # 这是一个例子
