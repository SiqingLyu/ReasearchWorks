...
# citynames = [ 'Beijing', 'Shanghai', 'Guangzhou']
# citynames = ['Shanghai']
# citynames = ['Zhang_Ukrine_WAR_2022']
# citynames = ['Zhang_Ukrine_WAR_2023']
'''--------------------------Configs-------------------------'''
...
def ClipVVVH_2dim(dataset_VVVH, outpath='VVVH.tif', offset_x=0,
    ...
    out_ds.GetRasterBand(1).SetNoDataValue(mn)
    # 写入目标文件
...
def ClipGT_1dim_GT(dataset_GT, outpath='GT.tif', offset_x=0,
    ...
    if if_test is True:
        if sum < 1:
            return False
    elif sum <= 0.05*block_ysize*block_ysize:
        return False
...
def ClipGT_1dim(dataset, outpath='1dim.tif', offset_x=0,
...
def make_city_dataset(city_img, fall_img, summer_img,spring_img, winter_img, VVVH_img, city_gt, height_img, img_outpath, spring_outpath,
                      fall_outpath, summer_outpath, winter_outpath, VVVH_outpath, all_path, gt_outpath, height_outpath, block_size):
    ...
    dataset_height = gdal.Open(height_img, gdal.GA_Update)
    dataset_height.FlushCache()
    ...
    x_gap_height, y_gap_height = cal_gaps(dataset_height, dataset_GT)
    ...
    with tqdm(total=int(xnum * ynum)) as pbar:
        for i in range(xnum):
            for j in range(ynum):
                ...
                try:
                    if ( ClipGT_1dim_GT(dataset_GT, outpath=gt_outpath + "_{0}_{1}.tif".format(i, j), offset_x=x,
                                    offset_y=y,
                                    block_xsize=block_size, block_ysize=block_size, if_height=False, if_test=False)
                    ):
                        pass

                        # Cliptif_4dim(dataset, outpath=img_outpath + "_{0}_{1}.tif".format(i, j), offset_x=x + x_gap,  # allyear
                        #              offset_y=y + y_gap, block_xsize=block_size, block_ysize=block_size)
                        # Cliptif_4dim(dataset_spring, outpath=spring_outpath + "_{0}_{1}.tif".format(i, j),      #  spring
                        #              offset_x=x + x_gap_spring,
                        #              offset_y=y + y_gap_spring, block_xsize=block_size, block_ysize=block_size)
                        # Cliptif_4dim(dataset_fall, outpath=fall_outpath + "_{0}_{1}.tif".format(i, j),          # autumn
                        #              offset_x=x + x_gap_fall,
                        #              offset_y=y + y_gap_fall, block_xsize=block_size, block_ysize=block_size)
                        # Cliptif_4dim(dataset_summer, outpath=summer_outpath + "_{0}_{1}.tif".format(i, j),      # summer
                        #              offset_x=x + x_gap_summer,
                        #              offset_y=y + y_gap_summer, block_xsize=block_size, block_ysize=block_size)
                        # Cliptif_4dim(dataset_winter, outpath=winter_outpath + "_{0}_{1}.tif".format(i, j),      # winter
                        #              offset_x=x + x_gap_winter,
                        #              offset_y=y + y_gap_winter, block_xsize=block_size, block_ysize=block_size)
                        ClipGT_1dim(dataset_height, outpath=height_outpath + "_{0}_{1}.tif".format(i, j),
                                    offset_x=x+x_gap_height,
                                    offset_y=y+y_gap_height, block_xsize=block_size, block_ysize=block_size)
                        # ClipVVVH_2dim(dataset_VVVH, outpath=VVVH_outpath + "_{0}_{1}.tif".format(i, j),         #VVH
                        #               offset_x= x + x_gap_VVVH,
                        #               offset_y= y + y_gap_VVVH, block_xsize=block_size, block_ysize=block_size)

                        # ClipVVVH_2dim_500(dataset_VVVH, outpath=VVVH_outpath + "_{0}_{1}.tif".format(i, j),  # VVH
                        #               offset_x=x + x_gap_VVVH,
                        #               offset_y=y + y_gap_VVVH, block_xsize=block_size, block_ysize=block_size)
                        # Cliptif_4dim2gray(dataset_spring, dataset_fall, dataset_summer, dataset_winter,       #mixed_seasons
...
def main():
    ...
    for i in range(len(citynames)):
        ...
        # gt = r"D:\Desktop\shanghai_masked" + '\\'  + citynames[i] + '.tif'
        ...
        # Height = r"D:\Dataset\CNBHChina62city51N" + '\\' + citynames[i] + '.tif'
        Height = r"D:\Dataset\WSF3DChina51N_height" + '\\' + citynames[i] + '.tif'
        Img_path.append(img)
        GT_path.append(gt)
    ...
    for i in range(len(citynames)):
        ...
        make_city_dataset(city_img=Img_path[i],
                          ...
                          height_img = height_path[i],
                          img_outpath=paths[3] + '\\'+ citynames[i].replace('_', ''),
                          spring_outpath = paths[10] + '\\'+ citynames[i].replace('_', ''),
                          fall_outpath=paths[4] + '\\' + citynames[i].replace('_', ''),
                          summer_outpath=paths[5] + '\\' + citynames[i].replace('_', ''),
                          winter_outpath=paths[6] + '\\' + citynames[i].replace('_', ''),
                          VVVH_outpath =paths[7] + '\\' + citynames[i].replace('_', ''),
                          gt_outpath=paths[8] + '\\' + citynames[i].replace('_', ''),
                          all_path=paths[9] + '\\' + citynames[i].replace('_', ''),
                          height_outpath=paths[11] + '\\' + citynames[i].replace('_', ''),
                          block_size=128
                          )
