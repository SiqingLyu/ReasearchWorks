...
def subtract_demicals(a, b):
    decimal_a = decimal.Decimal(a)
    decimal_b = decimal.Decimal(b)
    results = decimal_a - decimal_b
    return results
...
def gd_lonlat(lonlat1, lonlat2):
    return gd((lonlat1[1], lonlat1[0]), (lonlat2[1], lonlat2[0]))
...
class CBRALocator:
    ...
    def locate_image(self):
        ...
        lat_size = gd_lonlat(left_top, (left_top[0], right_bottom[1])).m  # 图像的纵向宽度
        # lon_size = gd_lonlat(right_bottom, (left_top[0], right_bottom[1])).m  # 图像的横向宽度， 以偏赤道的同一纬度为准
        lon_size = gd_lonlat(left_top, (right_bottom[0], left_top[1])).m  # 图像的横向宽度， 以偏赤道的同一纬度为准

        lat_size = int(np.ceil(lat_size / self.resolution))
        lon_size = int(np.ceil(lon_size / self.resolution))
        ...
        # lon_distance = gd_lonlat(left_top, (CBRA_name_lon, left_top[1])).m  # 计算CBRA左上角到索取范围左上角的横向距离
        lon_distance = gd_lonlat((CBRA_name_lon, CBRA_name_lat), (left_top[0], CBRA_name_lat)).m  # 计算CBRA左上角到索取范围左上角的横向距离

        lat_distance = gd_lonlat(left_top, (left_top[0], CBRA_name_lat)).m  # 计算CBRA左上角到索取范围左上角的纵向距离
        ...
        print(left_top[0], CBRA_name_lon, lon_index_inside)
        print(left_top[1], CBRA_name_lat, lat_index_inside)
        # 接下来定位超过边界的其他图像
        if (latitude_index_rb != latitude_index_lt) | (longitude_index_rb != longitude_index_lt):
            ...
            if latitude_index_rb == latitude_index_lt:  # 右上角
                ...
                lon_distance_lt2mid = gd_lonlat(left_top, (CBRA_name_lon_rt, left_top[1])).m  # 计算CBRA左上角到索取范围边界的横向距离
                ...
                lat_distance_lt2mid = gd_lonlat(left_top, (left_top[0], CBRA_name_lat_lb)).m  # 计算CBRA左上角到索取范围边界的纵向距离
                lat_index_inside_lt2mid = int(np.floor(lat_distance_lt2mid / self.resolution))  # 换算成像素行列数

                image_pixels_y_lt = lon_size
            ...
            image_pixels_y = lon_size
            image_pixels_x = lat_size
    ...
    def get_img_data(self):
        ...
        img_data = np.zeros((self.lat_size, self.lon_size), dtype=np.uint8)
        ...
        if self.locate_mode == 'RB':
            ...
            print(img_data.shape, lat_size_CBRA, lat_index_inside, lon_size_CBRA, lon_index_inside)
...
if __name__ == '__main__':
    CL = CBRALocator(file_root=r'G:\ProductData\CBRA', rectangle_pos=[[115.9, 41.4], [116.1, 41.2]])
    CL.locate_image()
    print(CL.CBRA_names)
    img_data = CL.get_img_data()
    tif.imsave(r'G:\ProductData\CBRA\CBRA_clip\Beijing.tif', img_data)
