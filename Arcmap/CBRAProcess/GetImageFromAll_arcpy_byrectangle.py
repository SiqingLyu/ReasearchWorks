...
！！！索取数据范围不应超过2.5°！！！
使用本代码时，只需要给程序输入所需要的长方形范围即可（左下角和右上角[longitude1, latitude1, longitude2, latitude2]）
输出：借助arcpy，直接得到裁剪之后的影像（索取范围内所有数据）
代码创建时间：2024.02.28
最后更新时间：2024.03.15
Lyu Siqing
"""
...
def main(root_path, save_path, save_name, year, Lon_min, Lat_min, Lon_max, Lat_max):
    ...
    CBRA_first = CBRA_names[0].split('\\')[-1][:-4]
    out_path = make_dir(os.path.join(save_path, str(year), CBRA_first))

    if (len(CBRA_names[1]) > 1) | (len(CBRA_names[2]) > 1) | (len(CBRA_names[3]) > 1):
        ...
        for ii in range(len(CBRA_names)):
            ...
            if len(CBRA_path) == 0:  # 如果对应的影像不存在
                continue
            out_raster = os.path.join(out_path_, save_name + '_{}.tif'.format(ii))
        ...
        if len(clipped_file_paths) > 1:
            ...
            arcpy.Mosaic_management(inputs[: -1], save_path_mosaic, mosaic_type="MAXIMUM", nodata_value=0)
            os.rename(save_path_mosaic,  os.path.join(out_path_, save_name + '.tif'))
...
if __name__ == '__main__':
    # Configs
    year = 2018
    root_path = r'F:\data\CBRA'
    save_path = r'F:\data\CBRA\CBRA_clip_86'

    # 读取Excel文件
    df = pd.read_excel(r"F:\data\polygon\point\point86.xlsx")

    # 提取经纬度范围列的数据
    excel_data = df[['Lon_min', 'Lat_min', 'Lon_max', 'Lat_max']].values.tolist()
    # Lon_min, Lat_min, Lon_max, Lat_max = 113.4, 41.2, 113.6, 41.4
    # save_name = "Beijing_{}_{}_{}_{}".format(Lon_min, Lat_min, Lon_max, Lat_max)  # 根据经纬度创建唯一的保存名称
    # main(root_path, save_path, save_name, year, Lon_min, Lat_min, Lon_max, Lat_max)

    # 输出经纬度范围数据并生成对应的TIFF文件
    for info in excel_data:
        Lon_min, Lat_min, Lon_max, Lat_max = info
        save_name = "Beijing_{}_{}_{}_{}".format(Lon_min, Lat_min, Lon_max, Lat_max)  # 根据经纬度创建唯一的保存名称
